...
- **2026-07-21 (Email provider → ZeptoMail HTTP API)**
  ...
  - **Send test email** — new [/api/admin/test-email](app/api/admin/test-email/route.js) (`settings:manage`) sends a sample through the real `sendEmail()` path (so it also lands in the Message Log with any provider error). Wired into **Settings → Templates & Config → Payment gateway**, under the Email status row: type a recipient → **Send test**. Verifies domain/token/DC in one click without a fake registration.
