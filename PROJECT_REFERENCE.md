...
- **2026-06-28**
  - **Refactor: split the giant admin file (phase 1, behaviour-preserving).** `app/admin/page.tsx` was 1893 lines. Extracted, with no functional change (verified: `tsc` clean + production build): shared **types** → `app/admin/types.ts`, **constants + `statusClasses`** → `app/admin/constants.ts`, and the two self-contained editors **`CategoryRow`** → `components/admin/CategoryRow.tsx` and **`EventRow`** → `components/admin/EventRow.tsx`. `page.tsx` is now 1535 lines. (Follow-up candidates: the registration detail modal, export helpers, and the ledger table — each to be done incrementally + verified.)
