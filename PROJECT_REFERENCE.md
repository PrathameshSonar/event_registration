...
- **2026-07-24 (About Us: rename + Pitham/Guruji sections + official-website nav button)**
  ...
  - **"Official Website" nav button** — external link (new tab) in the navbar (desktop + mobile), shown only when set. Stored as **`contact.website_url`** (Settings → Contact & Social, alongside the socials), passed to [LuxuryNavbar](components/site/LuxuryNavbar.js) as `officialUrl` from the (site) layout's existing `getContact()`. i18n `nav_official_site`.
