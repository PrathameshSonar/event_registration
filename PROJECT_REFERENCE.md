...
- **2026-07-21 (Entry wristbands + gate-first scan screen + form label overlap)**
  ...
  - **Scan screen rebuilt for the gate** ([app/entry/[id]/page.tsx](app/entry/[id]/page.tsx)): after the VALID/INVALID verdict it now leads with the **Seva name (large)**, a **big wristband-colour block**, and **"Bands to give" = attendees count** — the three things a volunteer acts on. Personal details moved below. Reads the mapping via the new cached `getEntryBands()` ([lib/settingsServer.js](lib/settingsServer.js)).
