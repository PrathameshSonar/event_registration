...
- **Enquiry sources:** a tier can be **Enquiry Only** (`is_enquiry_only`) or **Paid + Enquire** (`allow_enquiry` → shows both "Pay" and "Enquire Now" on the form). "Enquire Now" posts to `/api/enquiry` → `enquired` (holds no seat). **A third source: the homepage "Enquire Now"** (`POST /api/general-enquiry`, Settings → General Enquiry) creates a **Seva-less** lead (`category_id = null`) — badged "General enquiry", message shown, **Request Payment hidden** until a Seva is assigned. Independent of any Seva, so it collects interest pre-launch.
...
- `POST /api/general-enquiry` — homepage "Enquire Now" (Seva-independent). Creates a **category-less** `enquired` lead. Gated only by `general_enquiry.enabled`, never by registration-open/capacity. See §11 (Enquiries) + the 2026-07-23 changelog.
...
- **2026-07-24 (About Us: rename + Pitham/Guruji sections + official-website nav button)**
  - **Nav "About" → "About Us"** (`nav_about` in en/hi/mr; footer fallback; page `<title>`).
  - **One admin-editable section on `/about`** — about the **Pitham/temple** (per-language heading + body + one image; `AboutSection` in [AboutPageContent](components/site/pages/AboutPageContent.js), image shown at natural aspect — `h-auto`, no `object-cover` — so it's never cropped; hidden until it has content). New `app_settings.about_page` ([lib/appSettings.js](lib/appSettings.js)) + cached `getAboutPage()` ([lib/siteEvent.js](lib/siteEvent.js), tag `about_page`, keeps `/about` static `○`). Admin: Settings → **About Us Page** ([AboutPageManager](components/AboutPageManager.js), Website Content group). **Guruji has no dedicated section — the featured-guest "Guiding Light / Leadership" block already covers it.**
  - **"Official Website" link** — external (new tab), shown only when set. Stored as **`contact.website_url`** (Settings → Contact & Social, alongside the socials). Surfaced in **three places, deliberately NOT the desktop top nav** (it crowded the bar): the **footer** Contact column ([LuxuryFooter](components/site/LuxuryFooter.js)), the **mobile menu** ([LuxuryNavbar](components/site/LuxuryNavbar.js), which has vertical room), and a **button on the About Us page** ([AboutPageContent](components/site/pages/AboutPageContent.js)). The About page + navbar get the URL via `getContact()`. i18n `nav_official_site`.
  - **FAQ removed from the primary nav** (both desktop + mobile) — it still has its own `/faq` page, linked from the footer. Desktop nav is now Home · About Us · Event · Registration · Gallery · Contact.
  - No SQL — all new keys live in `app_settings`.
- **2026-07-24 (Homepage tidy + mobile language dropdown fix)**
  - **Removed the "See all Sevas" button** from the homepage tier section ([RegistrationCta](components/site/home/RegistrationCta.js)) — every Seva is already a clickable card in that same section, so it only linked back to a fuller copy of what's on screen. The pass-lookup link stays.
  - **🔴 Mobile: Marathi was unselectable in the language dropdown.** The mobile nav menu is an `overflow-hidden` max-height accordion, which clipped the dropdown at the card's bottom edge — cutting off the 3rd option. [LangToggle](components/LangToggle.js) gained a **`dropUp`** prop (opens the list *above* the button, `z-[60]`), used in the mobile menu so every language stays inside the visible card.
...
- **2026-07-23 (Homepage "Enquire Now" — Seva-independent lead capture)**
  - **A single always-available enquiry on the homepage**, so leads can be collected **before any Seva exists or is live** — the pre-launch ask. New `app_settings.general_enquiry` (`{ enabled (default true), title{en,hi,mr}, subtitle{en,hi,mr} }`, registry entry in [lib/appSettings.js](lib/appSettings.js)) + cached reader `getGeneralEnquiry()` ([lib/siteEvent.js](lib/siteEvent.js), tagged `general_enquiry`, so the homepage stays static `○` and a save applies immediately).
  - **Public:** [components/site/home/GeneralEnquiry.js](components/site/home/GeneralEnquiry.js) — a section (renders only when enabled) with an **Enquire Now** button opening a modal: first name, last name, mobile, email, optional message + T&C. Posts to **`POST /api/general-enquiry`**. Placed just before the closing CTA in [HomeContent](components/HomeContent.js). Trilingual (`enquiry_*` keys in en/hi/mr).
  - **The lead = a category-less `enquired` registration** (`category_id = null`, message in `problem_samasya`). This works because the whole Enquiries pipeline is already null-category-safe. It flows into the existing **Enquiries** tab (New → Contacted → notes → Close) with no schema change — `registrations.category_id` was already nullable.
  - **Deliberately NOT gated by `isRegistrationOpen` or a category** — the point is to collect interest when nothing is set up yet. Gated only by `general_enquiry.enabled`, **checked server-side** (read straight from the row, not the cache, so a just-flipped toggle is honoured on the write path). Light 10-minute same-phone idempotency stops double-submits.
  - **Admin:** Settings → **General Enquiry** ([GeneralEnquiryManager](components/GeneralEnquiryManager.js), mirrors DeclarationManager) — toggle + heading/subtext per language. In the Enquiries panel a Seva-less lead shows a blue **"General enquiry"** badge, its message, and **"No Seva selected"**; **Request Payment is hidden** for it (no price to charge — assign a Seva via Edit details first). `request-enquiry-payment` already refused a price-less row server-side.
  - ⚠️ **No new notifications** by design (the pipeline is the record). ⚠️ **Not** the contact form — that's `contact_messages` (a message inbox); this is a *lead* in the sales pipeline.
