...
- **2026-07-24 (About Us: rename + Pitham/Guruji sections + official-website nav button)**
  ...
  - **Two admin-editable sections on `/about`** — one about the **Pitham/temple**, one about **Guruji** — each with a per-language heading + body and one image (`AboutSection` in [AboutPageContent](components/site/pages/AboutPageContent.js), alternating image side, hidden until it has content). New `app_settings.about_page` ([lib/appSettings.js](lib/appSettings.js)) + cached `getAboutPage()` ([lib/siteEvent.js](lib/siteEvent.js), tag `about_page`, keeps `/about` static `○`). Admin: Settings → **About Us Page** ([AboutPageManager](components/AboutPageManager.js), Website Content group). Existing featured-guest "Leadership" block still renders below.
