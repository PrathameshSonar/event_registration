// Admin CRUD for an event's guest / artist / saint lineup.
...
const COLS = ['name', 'name_hi', 'role', 'role_hi', 'photo_url', 'bio', 'bio_hi', 'sort_order', 'translations'];
function pick(input = {}) {
    const out = {};
...
export async function GET(request) {
    ...
    const { data, error } = await supabaseAdmin.from('event_guests').select('*').eq('event_id', eventId).order('sort_order');
...
export async function POST(request) {
    ...
    const { data: cnt } = await supabaseAdmin.from('event_guests').select('id', { count: 'exact', head: true }).eq('event_id', body.event_id);
