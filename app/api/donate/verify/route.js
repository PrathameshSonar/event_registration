...
export async function POST(request) {
    ...
    if (donation.status !== 'completed') {
        ...
        if (donation.email) {
            await sendEmail({
                ...
                html: emailShell(`
                    <p style="font-size:16px;color:#404040;margin-top:0;">Namaste <strong>${escapeHtml(donation.name)}</strong>,</p>
    ...
    return NextResponse.json({ ok: true, name: donation.name, amount: donation.amount });
}
