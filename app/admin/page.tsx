...
import {
    ...
    Ticket, Calendar as CalendarIcon
} from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const handleCreateEvent = async (e: React.FormEvent) => {
        ...
        await supabase.from('events').insert([{
            title: newEventTitle,
            short_description: newEventShort,
            long_description: newEventLong,
            is_active: eventsList.length === 0
        }]);
    ...
    const handleUpdateCategory = async (id: string, updates: Partial<Category>) => {
        ...
        fetchAllData();
        setSaving(false);
        alert("Tier parameters updated successfully.");
    };

    const handleDeleteCategory = async (id: string) => {
        if (!confirm("Delete this category? Registrations tied to it will be preserved safely.")) return;
        setSaving(true);
    ...
    const filteredRegistrations = registrations.filter(reg => {
        ...
        let dateMatch = true;
        if (startDate) {
            dateMatch = dateMatch && new Date(reg.created_at) >= new Date(startDate);
        }
        if (endDate) {
            const end = new Date(endDate);
            end.setDate(end.getDate() + 1); // Include entire end day
            dateMatch = dateMatch && new Date(reg.created_at) < end;
        }
    ...
    // Calculate completed counts per category
    ...
    const downloadCSV = () => {
        const headers = [
            ...
            "Phone", "Email", "Pincode", "Taluka", "State", "Category",
            "Attendees", "Donation", "Total Paid", "Issue/Samasya", "Razorpay ID"
        ];
        const csvData = filteredRegistrations.map(reg => [
            ...
            reg.taluka || '',
            reg.state || '',
            reg.categories?.title || 'Deleted Tier',
        ...
        <div className="min-h-screen bg-neutral-50 p-4 md:p-8 font-sans">
                ...
                <div className="fixed inset-0 bg-neutral-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                        <div className="px-6 py-4 border-b border-neutral-200 flex justify-between items-center bg-neutral-50">
                            ...
                            <button onClick={() => setSelectedRegistration(null)} className="p-2 text-neutral-400 hover:text-red-600 transition rounded-full hover:bg-red-50">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
            ...
            <div className="max-w-7xl mx-auto mb-8 border-b border-neutral-200 pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-neutral-500 text-sm mt-1">Manage registrations, event configuration, and media galleries.</p>
                </div>
                <div className="flex bg-neutral-200 p-1 rounded-xl w-full md:w-auto">
                    <button
                        onClick={() => setActiveTab('registrations')}
                        className={`w-full md:w-auto px-5 py-2.5 text-sm font-semibold rounded-lg transition flex justify-center items-center gap-2 ${activeTab === 'registrations' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-600 hover:text-neutral-900 hover:bg-neutral-300/50'}`}
                    >
                        <ListFilter className="w-4 h-4" /> Registrations
                    </button>
                    <button
                        onClick={() => setActiveTab('settings')}
                        className={`w-full md:w-auto px-5 py-2.5 text-sm font-semibold rounded-lg transition flex justify-center items-center gap-2 ${activeTab === 'settings' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-600 hover:text-neutral-900 hover:bg-neutral-300/50'}`}
                    >
            ...
            <div className="max-w-7xl mx-auto">
                ...
                {/* ========================================= */}
                {/* TAB 1: REGISTRATIONS AND METRICS          */}
                {/* ========================================= */}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-white border border-neutral-200 p-6 rounded-xl flex items-center gap-4 shadow-sm">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-neutral-500">Confirmed Attendees</p>
                            ...
                            <div className="bg-white border border-neutral-200 p-6 rounded-xl flex items-center gap-4 shadow-sm">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-neutral-500">Filtered Revenue</p>
                            ...
                            <div className="bg-white border border-neutral-200 p-6 rounded-xl flex items-center gap-4 shadow-sm">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-neutral-500">Filtered Attempts</p>
                        ...
                        <div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
                            <h3 className="text-sm font-bold uppercase tracking-wider text-neutral-500 mb-4 flex items-center gap-2">
                                <Ticket className="w-4 h-4" /> Confirmed Sales per Category
                            </h3>
                            <div className="flex flex-wrap gap-4">
                                {Object.keys(categoryMetrics).length === 0 ? (
                                    <span className="text-sm text-neutral-400">No completed registrations match current filters.</span>
                                ) : (
                                    Object.entries(categoryMetrics).map(([cat, count]) => (
                                        <div key={cat} className="bg-neutral-50 border border-neutral-200 px-4 py-2 rounded-lg flex items-center gap-3">
                                            <span className="font-semibold text-neutral-700 text-sm">{cat}</span>
                                            <span className="bg-orange-100 text-orange-800 text-xs font-bold px-2.5 py-1 rounded-full">{count}</span>
                                        </div>
                                    ))
                                )}
                            </div>
                        ...
                        <div className="bg-white p-4 rounded-xl border border-neutral-200 flex flex-col lg:flex-row gap-4 justify-between items-center shadow-sm">
                            ...
                            <div className="flex flex-wrap lg:flex-nowrap gap-4 w-full lg:w-auto items-center">
                                ...
                                >
                                    ...
                                    {uniqueCategories.map((cat, idx) => <option key={idx} value={cat as string}>{cat}</option>)}
                                    <option value="Deleted"> [Deleted Tiers]</option>
                                </select>
                                ...
                                <button
                                    onClick={downloadCSV}
                                    className="bg-neutral-900 hover:bg-orange-600 text-white px-5 py-2.5 rounded-lg font-semibold flex items-center justify-center gap-2 text-sm transition"
                                >
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-sm whitespace-nowrap">
                                    ...
                                    <tbody className="divide-y divide-neutral-100">
                                        {loading ? (
                                            <tr><td colSpan={6} className="px-6 py-8 text-center text-neutral-400">Loading ledger data...</td></tr>
                                        ) : filteredRegistrations.length === 0 ? (
                                            <tr><td colSpan={6} className="px-6 py-8 text-center text-neutral-400">No records match your filters.</td></tr>
                                        ) : (
                                            filteredRegistrations.map((reg) => (
                                                <tr key={reg.id} className="hover:bg-neutral-50 transition">
                                                    ...
                                                    <td className="px-6 py-4 font-medium text-neutral-900">
                                                        ...
                                                        <br />
                                                        <span className="text-xs font-normal text-neutral-500">{reg.phone}</span>
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-4 text-center">
                                                        <button
                                                            ...
                                                            className="p-2 border border-neutral-200 rounded-lg bg-white hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 transition shadow-sm"
                                                        >
                    ...
                    <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-hidden flex flex-col md:flex-row min-h-[600px]">
                        ...
                        <div className="w-full md:w-64 bg-neutral-50 border-r border-neutral-200 p-4 space-y-2">
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 rounded-lg text-sm font-bold flex items-center gap-3 transition ${settingsSubTab === 'events' ? 'bg-orange-100 text-orange-700' : 'text-neutral-600 hover:bg-neutral-200'}`}
                            >
                                <CalendarDays className="w-4 h-4" /> Event Setup
                            </button>
                            <button
                                onClick={() => setSettingsSubTab('tiers')}
                                className={`w-full text-left px-4 py-3 rounded-lg text-sm font-bold flex items-center gap-3 transition ${settingsSubTab === 'tiers' ? 'bg-orange-100 text-orange-700' : 'text-neutral-600 hover:bg-neutral-200'}`}
                            >
                                <Ticket className="w-4 h-4" /> Ticket Tiers
                            </button>
                            <button
                                onClick={() => setSettingsSubTab('media')}
                                className={`w-full text-left px-4 py-3 rounded-lg text-sm font-bold flex items-center gap-3 transition ${settingsSubTab === 'media' ? 'bg-orange-100 text-orange-700' : 'text-neutral-600 hover:bg-neutral-200'}`}
                            >
                        ...
                        <div className="flex-1 p-6 lg:p-8 bg-white overflow-y-auto">
                            ...
                            {settingsSubTab === 'events' && (
                                <div className="max-w-3xl">
                                    ...
                                    <form onSubmit={handleCreateEvent} className="bg-neutral-50 p-6 rounded-xl border border-neutral-200 mb-8 space-y-4">
                                        ...
                                        <input
                                            type="text"
                                            placeholder="Event Title (e.g. Mahotsav 2026)"
                                            value={newEventTitle}
                                            onChange={(e) => setNewEventTitle(e.target.value)}
                                            className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-orange-600 text-sm"
                                            required
                                        />
                                        <textarea
                                            placeholder="Short Description (Appears on Home Page)"
                                            value={newEventShort}
                                            onChange={(e) => setNewEventShort(e.target.value)}
                                            className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-orange-600 resize-none text-sm"
                                            rows={2}
                                            required
                                        />
                                        <textarea
                                            placeholder="Long Description (Appears on Previous Events pages)"
                                            value={newEventLong}
                                            onChange={(e) => setNewEventLong(e.target.value)}
                                            className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-orange-600 resize-none text-sm h-24"
                                            required
                                        />
                                        <button
                                            type="submit"
                                            disabled={saving}
                                            className="bg-neutral-900 hover:bg-orange-600 text-white font-semibold px-6 py-2.5 rounded-lg text-sm transition"
                                        >
                                            Deploy Event
                                        </button>
                                    </form>
                                        ...
                                        {eventsList.map(ev => (
                                            <div key={ev.id} className={`p-5 rounded-xl border ${ev.is_active ? 'border-orange-300 bg-orange-50/50 shadow-sm' : 'border-neutral-200 bg-white'}`}>
                                                <div className="flex justify-between items-start mb-2">
                                                    <h4 className="font-bold text-lg text-neutral-900">{ev.title}</h4>
                                                    <div className="flex items-center gap-2">
                                                        {ev.is_active ? (
                                                            <span className="bg-orange-600 text-white text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded">Active Homepage</span>
                                                        ) : (
                                                            <button
                                                                onClick={() => handleSetEventActive(ev.id)}
                                                                className="text-xs font-semibold border border-neutral-300 px-3 py-1.5 rounded-lg hover:bg-neutral-100 transition"
                                                            >
                                                                Set Active
                                                            </button>
                                                        )}
                                                        <button
                                                            ...
                                                            className="text-neutral-400 hover:text-red-600 p-1.5 border border-transparent hover:border-red-200 rounded-lg hover:bg-red-50 transition"
                                                            title="Delete Event"
                                                        >
                            ...
                            {settingsSubTab === 'tiers' && (
                                <div className="max-w-4xl">
                                    ...
                                    <form onSubmit={handleCreateCategory} className="flex flex-col md:flex-row gap-4 mb-8 bg-neutral-50 p-6 rounded-xl border border-neutral-200">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className="w-full md:w-32 px-3 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-orange-600 text-sm"
                                                required
                                            />
                                        </div>
                                        <div className="flex items-end">
                                            <button
                                                type="submit"
                                                disabled={saving}
                                                className="w-full bg-neutral-900 hover:bg-orange-600 text-white text-sm font-semibold px-6 py-2 rounded-lg transition h-[38px] whitespace-nowrap"
                                            >
                                                Add New Tier
                                            </button>
                                        </div>
                                ...
                                <div className="max-w-4xl">
                                    ...
                                    <form onSubmit={handleAddMedia} className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-neutral-50 p-6 border border-neutral-200 rounded-xl mb-8">
                                        <div className="md:col-span-2">
                                            <label className="block text-xs font-bold text-neutral-600 uppercase tracking-wider mb-1">Asset URL</label>
                                            <input
                                                type="url"
                                                placeholder="https://..."
                                                value={mediaUrl}
                                                onChange={(e) => setMediaUrl(e.target.value)}
                                                className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:border-orange-600"
                                                required
                                            />
                                        </div>
                                        <div className="md:col-span-1">
                                            <label className="block text-xs font-bold text-neutral-600 uppercase tracking-wider mb-1">Media Type</label>
                                            <select
                                                value={mediaType}
                                                onChange={(e) => setMediaType(e.target.value as 'image' | 'youtube')}
                                                className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg bg-white focus:outline-none focus:border-orange-600 cursor-pointer"
                                            >
                                        ...
                                        <div className="md:col-span-1">
                                            <label className="block text-xs font-bold text-neutral-600 uppercase tracking-wider mb-1">Link Event</label>
                                            <select
                                                value={mediaEventId}
                                        ...
                                        <div className="md:col-span-3">
                                            <label className="block text-xs font-bold text-neutral-600 uppercase tracking-wider mb-1">Caption / Subtext</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="md:col-span-1 flex items-end">
                                            <button
                                                type="submit"
                                                disabled={saving}
                                                className="w-full bg-neutral-900 hover:bg-orange-600 text-white font-bold py-2.5 rounded-lg transition flex justify-center items-center gap-2"
                                            >
                                    ...
                                    <div className="space-y-4">
                                        {mediaList.map((media) => (
                                            <div key={media.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border border-neutral-200 rounded-xl bg-white shadow-sm gap-4 hover:border-neutral-300 transition">
                                                <div className="flex items-center gap-4 w-full">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="flex items-center gap-2 mt-1.5">
                                                            <span className="uppercase tracking-wider font-bold text-[10px] bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded border border-neutral-200">
                                                                {media.media_type}
                                                            </span>
                                                            <span className="text-xs text-neutral-500">
                                                                Linked to: {media.events?.title || 'Unknown Event'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <button
                                                    ...
                                                    className="text-neutral-400 hover:text-red-600 p-2 border border-transparent hover:border-red-200 rounded-lg bg-neutral-50 hover:bg-red-50 transition"
                                                    title="Delete Media"
                                                >
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                        ...
                                        {mediaList.length === 0 && (
                                            <div className="p-8 text-center text-neutral-400 border border-dashed border-neutral-300 rounded-xl bg-neutral-50">
                                                No media assets found in the repository. Add an image or YouTube link above.
                                            </div>
                                        )}
                                    </div>
                                </div>
...
// ============================================================================
// COMPONENT: CategoryRow (Handles Local State & Explicit "Update" Button)
// ============================================================================
    ...
    return (
        <div className="border border-neutral-200 rounded-xl p-5 bg-white shadow-sm relative transition-all hover:border-neutral-300">
            ...
            <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex items-center gap-3">
                    <select
                        ...
                        onChange={(e) => { setIsFull(e.target.value === 'full'); setIsChanged(true); }}
                        className={`text-xs border rounded-lg px-2.5 py-1.5 font-bold cursor-pointer outline-none transition ${isFull ? 'bg-red-50 text-red-700 border-red-200' : 'bg-green-50 text-green-700 border-green-200'}`}
                    >
                    ...
                    <button
                        ...
                        className="text-neutral-400 hover:text-red-600 p-1.5 border border-transparent hover:border-red-200 rounded bg-neutral-50 hover:bg-red-50 transition"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Fee Amount (₹)</label>
                    <input
                        type="number"
                        value={price}
                        onChange={(e) => { setPrice(Number(e.target.value)); setIsChanged(true); }}
                        className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg bg-neutral-50 focus:outline-none focus:border-orange-500 focus:bg-white transition"
                    />
                </div>
                <div>
                    <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Thumbnail Media URL</label>
                    <input
                        type="text"
                        placeholder="https://..."
                        value={mediaUrl}
                        onChange={(e) => { setMediaUrl(e.target.value); setIsChanged(true); }}
                        className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg bg-neutral-50 focus:outline-none focus:border-orange-500 focus:bg-white transition"
                    />
                </div>
                <div className="md:col-span-2">
                    <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Short Summary (Home Page)</label>
                    <input
                        type="text"
                        value={desc}
                        onChange={(e) => { setDesc(e.target.value); setIsChanged(true); }}
                        className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg bg-neutral-50 focus:outline-none focus:border-orange-500 focus:bg-white transition"
                    />
                </div>
                <div className="md:col-span-2">
                    <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Detailed Description (Perks)</label>
                    <textarea
                        value={detailedDesc}
                        onChange={(e) => { setDetailedDesc(e.target.value); setIsChanged(true); }}
                        className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg bg-neutral-50 focus:outline-none focus:border-orange-500 focus:bg-white transition h-20 resize-none"
                    />
                </div>
            </div>

            {/* EXPLICIT UPDATE BUTTON ROW */}
            <div className="mt-5 pt-4 border-t border-neutral-100 flex justify-end">
                <button
                    onClick={handleUpdateClick}
                    disabled={!isChanged}
                    className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all ${isChanged ? 'bg-orange-600 text-white shadow-md hover:bg-orange-700' : 'bg-neutral-100 text-neutral-400 cursor-not-allowed border border-neutral-200'}`}
                >
                    <Save className="w-4 h-4" />
                    {isChanged ? "Commit Updates" : "Up to date"}
                </button>
            </div>
