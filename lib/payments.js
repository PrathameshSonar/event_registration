...
export async function ensureBalanceLink(reg) {
    ...
    try {
        ...
        const link = await getRazorpayClient().paymentLink.create({
            ...
            // MUST be unique per creation: Razorpay rejects a duplicate reference_id,
            // and a link may already exist for this registration (minted at advance
            // capture, or orphaned after the stored URL was cleared by a donation
            // adjustment). Same reason resend-balance timestamps its reference.
