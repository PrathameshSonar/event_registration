...
export async function sendPaymentLink(reg, kind = 'balance') {
    ...
    const subject = isEnquiry
        ? '🙏 Complete your registration — BaglaBhairav'
        : '⏳ Pay your balance — BaglaBhairav registration';
    const eCat = escapeHtml(categoryTitle);
    const eName = `${escapeHtml(reg.first_name)} ${escapeHtml(reg.last_name)}`;
    const lead = isEnquiry
        ? `Thank you for your interest in <strong>${eCat}</strong>. To confirm your registration and receive your entry pass, please complete the payment:`
        : `Thank you — your advance of <strong>₹${Number(reg.amount_paid).toLocaleString('en-IN')}</strong> for <strong>${eCat}</strong> is received. To confirm your registration and receive your entry pass, please clear the remaining balance:`;
    const amountLabel = isEnquiry ? 'Amount payable' : 'Balance due';
    const cta = isEnquiry ? 'Pay & Confirm' : 'Pay Balance Now';
                ...
                html: `
                    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;">
                      ...
                      <div style="padding:32px;background:#fff;">
                        ...
                        <p style="font-size:14px;color:#6b7280;line-height:1.6;">${lead}</p>
                        <div style="text-align:center;margin:24px 0;">
                          <div style="font-size:13px;color:#6b7280;">${amountLabel}</div>
                          <div style="font-size:28px;font-weight:800;color:#ea580c;margin:4px 0 16px;">₹${dueRupees.toLocaleString('en-IN')}</div>
                          <a href="${shortUrl}" style="display:inline-block;background:#ea580c;color:#fff;font-weight:700;padding:14px 28px;border-radius:10px;text-decoration:none;">${cta}</a>
                        </div>
    ...
    if (reg.phone && process.env.WHATSAPP_API_URL && process.env.WHATSAPP_ACCESS_TOKEN) {
        try {
            let cleanPhone = String(reg.phone).replace(/\D/g, '');
            if (cleanPhone.length === 10) cleanPhone = `91${cleanPhone}`;
            const text = isEnquiry
                ? `🙏 *BaglaBhairav* — to confirm your registration for *${categoryTitle}*:\n\n*Amount payable: ₹${dueRupees.toLocaleString('en-IN')}*\nPay here:\n${shortUrl}\n\nYour entry pass is issued after payment.`
                : `🙏 *BaglaBhairav* — advance received for *${categoryTitle}*.\n\n*Balance due: ₹${dueRupees.toLocaleString('en-IN')}*\nPay here to confirm your registration:\n${shortUrl}\n\nYour entry pass is issued only after full payment.`;
        ...
        } catch (e) { console.error('🚨 Payment link WhatsApp failed:', e); }
    }
    return shortUrl;
}
