...
const DEFAULT_SITE_NAME = DEFAULT_BRANDING.site_name;
...
export async function sendTemplatedEmail({ to, kind, vars = {}, attachments = null, registrationId = null, metadata = null }) {
    ...
    // `siteName` is injected into EVERY template automatically, so no template —
    // shipped or admin-edited — ever has to hardcode the brand name. A caller can
    // still override it, but nothing needs to. Rename in Settings → Branding & SEO
    // and the next email carries the new name.
    let brand = { site_name: DEFAULT_SITE_NAME, brand_line2: '' };
    try { brand = await getBranding(); } catch { /* keep the default */ }
    const siteName = (brand?.site_name || '').trim() || DEFAULT_SITE_NAME;
    vars = { siteName, ...vars };
    ...
    return sendEmail({
        ...
        html: wrap ? emailShell(rendered, siteName, brand?.brand_line2) : rendered,
        attachments,
...
//
// `siteName` comes from Settings → Branding & SEO (`getSiteName()` server-side).
// It is a parameter rather than a read inside this function because emailShell is
// synchronous and called from render paths; every caller already has the name.
// The optional `line2` is the wordmark's second line (`branding.brand_line2`) —
// blank by default, so an unconfigured site shows just the name.
export function emailShell(inner, siteName = DEFAULT_SITE_NAME, line2 = '') {
    const name = escapeHtml(String(siteName || DEFAULT_SITE_NAME));
    const sub = escapeHtml(String(line2 || '').trim());
    return `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;">
        <div style="background:#171717;padding:24px;text-align:center;"><span style="color:#ea580c;font-size:12px;font-weight:bold;text-transform:uppercase;">${name}</span>${sub ? `<div style="color:#fff;font-size:22px;font-weight:800;margin-top:4px;">${sub}</div>` : ''}</div>
