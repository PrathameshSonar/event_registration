...
// ── The site name, for everything that is NOT CSS ────────────────────────────
// Emails, WhatsApp bodies, Razorpay checkout/payment-link descriptions, receipts,
// export filenames, the scanner header, JSON-LD. Every one of those used to
// hardcode the brand name, so a rename left the old one scattered across the
// product — visible to customers in their inbox and on their receipt.
//
// SERVER ONLY (this file imports supabaseAdmin). Client components read the same
// value from `useBranding()` — the root layout already feeds it the same object.
// Cached + tagged 'branding', so a save in Settings → Branding & SEO takes effect
// immediately rather than waiting out the hour.
export async function getSiteName() {
    try {
        const b = await getBranding();
        return (b?.site_name || '').trim() || DEFAULT_BRANDING.site_name;
    } catch {
