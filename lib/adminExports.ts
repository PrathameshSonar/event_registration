...
export function downloadRegistrationsCsv(rows: Registration[]) {
    ...
    download(new Blob(['﻿' + csvContent], { type: 'text/csv;charset=utf-8;' }), `BaglaBhairav_Registrations_${today()}.csv`);
}

// Excel export — a real .xls (Excel-native HTML table), no extra dependency.
export function downloadRegistrationsExcel(rows: Registration[]) {
    ...
    download(new Blob(['﻿' + html], { type: 'application/vnd.ms-excel' }), `BaglaBhairav_Registrations_${today()}.xls`);
}

// Combined receipts (PDF) — a print-friendly window of all PAID rows (one per page).
export function printReceiptsPdf(rows: Registration[], eventTitle: string) {
    ...
    const receipts = paid.map(reg => {
        ...
        return `<div class="rcpt">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #171717;padding-bottom:12px;margin-bottom:16px;">
                <div><div style="font-size:22px;font-weight:800;">BaglaBhairav</div><div style="font-size:12px;color:#6b7280;">${esc(eventTitle)}</div></div>
...
export function downloadFinancialStatement(rows: Registration[]) {
    const paid = rows.filter(r => r.payment_status === 'completed');
    if (paid.length === 0) { toast.error('No paid registrations in the current filter.'); return; }
    ...
    download(new Blob(['﻿' + html], { type: 'application/vnd.ms-excel' }), `BaglaBhairav_Financial_${today()}.xls`);
}
