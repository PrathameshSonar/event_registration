        ...
        try {
            await getResend().emails.send({
                ...
                html: shell(`
                    ...
                    <p style="font-size:14px;color:#6b7280;line-height:1.6;">We've received your <strong>${method}</strong> payment details for <strong>${categoryTitle}</strong>${reg.offline_reference ? ` (Ref: ${reg.offline_reference})` : ''}. Our team will verify it and confirm your registration shortly. Your entry pass is issued once the payment is verified.</p>
                `),
            });
    ...
    if (reg.email && process.env.RESEND_API_KEY) {
        try {
            await getResend().emails.send({
                ...
                html: shell(`
                    <p style="font-size:16px;color:#404040;margin-top:0;">Namaste <strong>${reg.first_name} ${reg.last_name}</strong>,</p>
                    <p style="font-size:14px;color:#6b7280;line-height:1.6;">We couldn't verify your payment for <strong>${categoryTitle}</strong>.</p>
                    ${reason ? `<p style="font-size:14px;color:#b91c1c;line-height:1.6;"><strong>Reason:</strong> ${reason}</p>` : ''}
