...
// Settings → Admin Users. Create named admin/viewer accounts, reset passwords,
// enable/disable, and remove them. Named accounts let the audit log record WHO
// acted (vs. the shared-password login, which only records the role).
"use client";

import { useEffect, useState } from "react";
import { UserPlus, Trash2, KeyRound, Power } from "lucide-react";
...
export default function AdminUsersManager() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h3 className="text-lg font-bold text-neutral-900">Admin Users</h3>
                <p className="text-sm text-neutral-500">Named accounts so the audit log shows who did what. The shared password (from env) still works with a blank username.</p>
            </div>

            <form onSubmit={create} className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                ...
                <div><label className="text-xs font-semibold text-neutral-500 mb-1 block">Role</label>
                    <select className={input + " w-full"} value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}><option value="admin">Admin</option><option value="viewer">Viewer</option></select>
                </div>
                <button type="submit" disabled={busy} className="bg-orange-600 text-white rounded-lg px-4 py-2 text-sm font-bold hover:bg-orange-700 disabled:opacity-50 flex items-center justify-center gap-2"><UserPlus className="w-4 h-4" /> Add</button>
            </form>

            <div className="border border-neutral-200 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                    <thead className="bg-neutral-50 text-neutral-500 text-xs uppercase tracking-wider">
                        <tr><th className="text-left px-4 py-3">User</th><th className="text-left px-4 py-3">Role</th><th className="text-left px-4 py-3">Status</th><th className="text-left px-4 py-3">Last login</th><th className="text-right px-4 py-3">Actions</th></tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-100">
                        {loading ? (
                            ...
                            <tr key={u.id} className="hover:bg-neutral-50">
                                ...
                                <td className="px-4 py-3"><span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-bold border ${u.role === "admin" ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-neutral-100 text-neutral-600 border-neutral-200"}`}>{u.role}</span></td>
                                ...
                                <td className="px-4 py-3 text-neutral-500">{u.last_login_at ? new Date(u.last_login_at).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "—"}</td>
